import { RiskLevel, OperationType } from './types.js';
...
export class TrustDecisionEngine {
    ...
    async evaluateOperation(operation) {
        ...
        try {
            ...
            const policyResult = await this.policyEvaluator.evaluateAgainstPolicy(operation, classification, policy);
            // 5. 最終判定
     ...
     * 要件4.1, 4.2に基づいて実装
     */
    async logOperationResult(operation, decision, executionResult, user, sessionId) {
        try {
            if (decision.requiresManualApproval) {
                // 手動承認操作のログ記録
                await this.auditLogger.logManualApproval(operation, decision, executionResult, user, sessionId);
            }
            else {
                // 自動承認操作のログ記録
                await this.auditLogger.logAutoApproval(operation, decision, executionResult, user, sessionId);
            }
        }
        catch (error) {
     ...
     * セキュリティアクションを処理する
     */
    async handleSecurityAction(action, operation, securityResult) {
        switch (action) {
     ...
     * 最終判定を行う
     */
    makeFinalDecision(operation, classification, policyResult, securityResult) {
     ...
     * パフォーマンスを記録する
     */
    recordPerformance(startTime, result) {
...
class SecurityVerifier {
     ...
     * 外部からの不正操作要求をチェックする
     */
    checkExternalRequest(operation) {
        // セッションIDの検証
...
class PolicyEvaluator {
    async evaluateAgainstPolicy(operation, classification, policy) {
        try {
            // 自動承認対象かチェック
    ...
    isAutoApproveOperation(operation, classification, policy) {
        ...
        if (classification.operationType === OperationType.GIT) {
            return policy.autoApprove.gitOperations.some(op => args.includes(op) || args.join(' ').includes(op));
        }
        // ファイル操作のチェック
        if (classification.operationType === OperationType.FILE) {
            return policy.autoApprove.fileOperations.some(op => command === op || command.includes(op));
        }
        // CLI操作のチェック
        ...
        if (classification.operationType === OperationType.SCRIPT) {
            const hasAllowedExtension = args.some(arg => policy.autoApprove.scriptExecution.extensions.some(ext => arg.endsWith(ext)));
            const hasAllowedPath = args.some(arg => policy.autoApprove.scriptExecution.allowedPaths.some(path => arg.startsWith(path)));
            return hasAllowedExtension || hasAllowedPath;
        }
        return false;
    }
    isManualApproveOperation(operation, classification, policy) {
        const fullCommand = `${operation.command} ${operation.args.join(' ')}`;
        // 削除系操作のチェック
        const isDeletion = policy.manualApprove.deleteOperations.some(pattern => this.matchesPattern(fullCommand, pattern));
        // 強制系操作のチェック
        const isForce = policy.manualApprove.forceOperations.some(pattern => this.matchesPattern(fullCommand, pattern));
        // 本番影響操作のチェック
        const isProductionImpact = policy.manualApprove.productionImpact.some(pattern => this.matchesPattern(fullCommand, pattern));
