...
export class ReportGenerator {
  ...
  constructor(reportsDir: string = '.kiro/reports') {
   ...
   */
  async generatePolicyUpdateReport(
    previousPolicy: TrustPolicy,
    newPolicy: TrustPolicy,
    generatedBy: string = 'system'
  ): Promise<PolicyUpdateReport> {
    ...
    const report: PolicyUpdateReport = {
      id: reportId,
      timestamp,
      previousPolicy,
      newPolicy,
   ...
   * ポリシー間の変更点を分析
   */
  private analyzeChanges(previousPolicy: TrustPolicy, newPolicy: TrustPolicy): PolicyChange[] {
   ...
   * 自動承認設定の変更を分析
   */
  private analyzeAutoApproveChanges(previous: any, current: any): PolicyChange[] {
    const changes: PolicyChange[] = [];
    ...
    for (const [tool, operations] of Object.entries(cliChanges.added)) {
      changes.push({
        ...
        description: `${tool} CLI操作に自動承認を追加: ${(operations as string[]).join(', ')}`
      });
    }
    for (const [tool, operations] of Object.entries(cliChanges.removed)) {
      changes.push({
        ...
        description: `${tool} CLI操作から自動承認を削除: ${(operations as string[]).join(', ')}`
      });
    }
   ...
   * 手動承認設定の変更を分析
   */
  private analyzeManualApproveChanges(previous: any, current: any): PolicyChange[] {
    const changes: PolicyChange[] = [];
   ...
   * セキュリティ設定の変更を分析
   */
  private analyzeSecurityChanges(previous: any, current: any): PolicyChange[] {
    const changes: PolicyChange[] = [];
   ...
   * 影響範囲を分析
   */
  private analyzeImpact(changes: PolicyChange[], previousPolicy: TrustPolicy, newPolicy: TrustPolicy): ImpactAnalysis {
   ...
   * セキュリティ影響を分析
   */
  private analyzeSecurityImpact(changes: PolicyChange[]): SecurityImpact {
   ...
   * パフォーマンス影響を分析
   */
  private analyzePerformanceImpact(changes: PolicyChange[], previousPolicy: TrustPolicy, newPolicy: TrustPolicy): PerformanceImpact {
   ...
   * ユーザーエクスペリエンス影響を分析
   */
  private analyzeUserExperienceImpact(changes: PolicyChange[]): UserExperienceImpact {
    let trustDialogFrequencyChange = 0;
    let workflowDisruptionLevel: 'none' | 'minimal' | 'moderate' | 'significant' = 'none';
    const descriptions: string[] = [];
    ...
    for (const change of changes) {
      ...
      if (change.section === 'manualApprove' && change.changeType === 'added') {
        ...
        if (workflowDisruptionLevel === 'none') workflowDisruptionLevel = 'minimal';
      }
   ...
   * 期待効果を分析
   */
  private analyzeExpectedEffects(changes: PolicyChange[], impactAnalysis: ImpactAnalysis): ExpectedEffect[] {
   ...
   * レポートをファイルに保存
   */
  private async saveReportToFile(report: PolicyUpdateReport): Promise<void> {
    try {
      // レポートディレクトリを作成
   ...
   * Markdownレポートを生成
   */
  private generateMarkdownReport(report: PolicyUpdateReport): string {
...
`;
  ...
  private compareArrays(arr1: string[], arr2: string[]): { added: string[], removed: string[] } {
  ...
  private compareObjects(obj1: Record<string, string[]>, obj2: Record<string, string[]>): { added: Record<string, string[]>, removed: Record<string, string[]> } {
  ...
  private getAffectedOperations(changes: PolicyChange[]): string[] {
  ...
  private countAutoApprovalOperations(policy: TrustPolicy): number {
    let count = 0;
  ...
  private getDisruptionLevelLabel(level: string): string {
    const labels = {
      'none': 'なし',
      'minimal': '最小限',
      'moderate': '中程度',
  ...
  private getEffectCategoryLabel(category: string): string {
    const labels = {
      'security': 'セキュリティ',
      'performance': 'パフォーマンス',
      'usability': 'ユーザビリティ',
