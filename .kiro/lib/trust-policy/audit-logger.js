...
export class AuditLogger {
     ...
     */
    async logAutoApproval(operationOrData, decision, executionResult, user = 'system', sessionId = this.generateSessionId()) {
        // テスト用の簡易形式をサポート
        if (typeof operationOrData.operation === 'string') {
            ...
            try {
                ...
                await this.logError('auto-approval-log-failed', error, log);
            }
            return;
        }
        // 通常の形式
        const operation = operationOrData;
        const log = {
            id: this.generateLogId(),
            timestamp: new Date(),
            operation,
     ...
     */
    async logManualApproval(operationOrData, decision, executionResult, user = 'user', sessionId = this.generateSessionId()) {
        // テスト用の簡易形式をサポート
        if (typeof operationOrData.operation === 'string') {
            const data = operationOrData;
            const log = {
            ...
            try {
                ...
                await this.logError('manual-approval-log-failed', error, log);
            }
            return;
        }
        // 通常の形式
     ...
     * 自動承認ログファイルに書き込み
     */
    async writeAutoApprovalLog(log) {
     ...
     * 手動承認ログファイルに書き込み
     */
    async writeManualApprovalLog(log) {
        const date = this.formatDate(log.timestamp);
   ...
   * ログローテーション機能
     ...
     * 自動承認ログエントリのフォーマット
     */
    formatAutoApprovalLogEntry(log) {
...
`;
     ...
     * 手動承認ログエントリのフォーマット
     */
    formatManualApprovalLogEntry(log) {
        const timestamp = log.timestamp.toISOString();
...
`;
     ...
     * エラーログエントリのフォーマット
     */
    formatErrorLogEntry(type, error, context) {
...
`;
     ...
     * ログファイルのヘッダー生成
     */
    generateLogFileHeader(filePath) {
...
`;
     ...
     * セキュリティイベントの統計を取得する
     */
    async getSecurityEventStats(days = 7) {
        const stats = {
            securityEvents: 0,
    ...
    async rotateLogs() {
        try {
            const files = await fs.readdir(this.config.reportsDir);
            const logFiles = files.filter(file => file.match(/^(auto|manual|trust-error)-trust-log-.*\.md$/));
            for (const file of logFiles) {
                const filePath = join(this.config.reportsDir, file);
