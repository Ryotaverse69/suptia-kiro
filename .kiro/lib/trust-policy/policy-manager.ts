...
export class PolicyManager {
  private static readonly POLICY_FILE_PATH = '.kiro/settings/trust-policy.json';
  private static readonly DEFAULT_POLICY_PATH = '.kiro/lib/trust-policy/default-policy.json';
  
  private cachedPolicy: TrustPolicy | null = null;
   ...
   * デフォルトポリシーを読み込む
   */
  async loadDefaultPolicy(): Promise<TrustPolicy> {
    try {
      const defaultContent = await fs.readFile(PolicyManager.DEFAULT_POLICY_PATH, 'utf-8');
      const defaultPolicy = JSON.parse(defaultContent) as TrustPolicy;
   ...
   * ポリシー設定を更新する
   */
  async updatePolicy(policy: TrustPolicy, generateReport: boolean = true): Promise<void> {
    // 更新前に検証
    const validation = this.validatePolicy(policy);
    if (!validation.isValid) {
    ...
    try {
      ...
      if (generateReport && previousPolicy) {
        try {
          await this.reportGenerator.generatePolicyUpdateReport(
            previousPolicy,
            updatedPolicy,
            'policy-manager'
          );
   ...
   * ポリシー更新レポートを手動生成する
   */
  async generateUpdateReport(previousPolicy: TrustPolicy, newPolicy: TrustPolicy, generatedBy: string = 'manual'): Promise<void> {
    try {
   ...
   */
  private getHardcodedDefaultPolicy(): TrustPolicy {
    return {
      version: "1.0",
   ...
   * レポート生成の自動化設定を確認
   */
  async isReportGenerationEnabled(): Promise<boolean> {
    try {
   ...
   * レポート生成のエラーハンドリング
   */
  private async handleReportGenerationError(error: Error, context: string): Promise<void> {
