import { Operation, OperationType, RiskLevel, ClassificationResult } from './types.js';
...
export class OperationClassifier {
  ...
  private readonly AUTO_APPROVE_SCRIPT_EXTENSIONS = ['.mjs', '.js'];
   ...
   * 要件2.1-2.4に基づいて操作を分類
   */
  private determineOperationType(operation: Operation): OperationType {
   ...
   * ファイル操作かどうかを判定（要件2.2）
   */
  private isFileOperation(command: string, args: string[]): boolean {
    // 明示的なファイル操作コマンド
   ...
   * CLI操作かどうかを判定（要件2.3）
   */
  private isCliOperation(command: string, args: string[]): boolean {
    const cliTools = [
   ...
   * 強化されたスクリプト検出ロジック
   */
  private isScriptExecution(command: string, args: string[]): boolean {
    try {
      // 自動承認対象のスクリプト拡張子
      const hasAutoApproveScript = args.some(arg => 
        this.AUTO_APPROVE_SCRIPT_EXTENSIONS.some(ext => arg.endsWith(ext))
      );
      ...
      // 自動承認対象のスクリプトパス
      const hasAutoApproveScriptPath = args.some(arg =>
        this.AUTO_APPROVE_SCRIPT_PATHS.some(path => arg.startsWith(path))
      );
   ...
   * MCP操作かどうかを判定
   */
  private isMcpOperation(operation: Operation): boolean {
   ...
   * リスクレベルを評価する
   */
  private assessRiskLevel(operation: Operation, operationType: OperationType): RiskLevel {
    // 削除系操作は高リスク
   ...
   * 強化された削除操作検出ロジック
   */
  private isDeletionOperation(operation: Operation): boolean {
    ...
    try {
      ...
      const hasDeleteKeyword = args.some(arg => 
        deletionKeywords.some(keyword => arg.includes(keyword))
      );
   ...
   * 強化された強制操作検出ロジック
   */
  private isForceOperation(operation: Operation): boolean {
    const command = operation.command.toLowerCase();
    const args = operation.args.map(arg => arg.toLowerCase());
    const originalArgs = operation.args; // 大文字小文字を保持
    ...
    try {
      ...
      const hasForceFlag = originalArgs.some(arg => 
        forceKeywords.some(keyword => arg === keyword || arg.includes(keyword))
      );
   ...
   * 強化された本番環境影響操作検出ロジック
   */
  private isProductionImpactOperation(operation: Operation): boolean {
    const command = operation.command.toLowerCase();
    const args = operation.args.map(arg => arg.toLowerCase());
    ...
    try {
      ...
      if (operation.context?.mcpServer && operation.context?.mcpTool) {
        ...
        for (const pattern of this.PRODUCTION_IMPACT_PATTERNS) {
          if (pattern.mcpServer && pattern.mcpServer === server) {
            const hasWriteOperation = pattern.writeOperations?.some(op => 
              tool.includes(op)
            );
      ...
      if (command === 'vercel') {
        ...
        const hasProductionFlag = args.some(arg => 
          productionFlags.some(flag => arg.includes(flag))
        );
      ...
      if (command === 'aws') {
        ...
        const hasAwsProductionOperation = args.some(arg =>
          awsProductionKeywords.some(keyword => arg.includes(keyword))
        );
   ...
   * 破壊的なファイル操作かどうかを判定
   */
  private isDestructiveFileOperation(operation: Operation): boolean {
    const command = operation.command.toLowerCase();
   ...
   * MCP書き込み操作かどうかを判定
   */
  private isMcpWriteOperation(operation: Operation): boolean {
    const tool = operation.context?.mcpTool?.toLowerCase() || '';
   ...
   * 書き込み操作かどうかを判定
   */
  private isWriteOperation(tool: string): boolean {
    const writePatterns = [
   ...
   * マッチしたパターンを取得
   */
  private getMatchedPatterns(operation: Operation): string[] {
   ...
   * 強化された自動承認パターン検出ロジック
   */
  private isAutoApprovePattern(operation: Operation): boolean {
    const command = operation.command.toLowerCase();
    const args = operation.args.map(arg => arg.toLowerCase());
    const argsString = args.join(' ');
    ...
    try {
      ...
      if (command === 'git') {
        ...
        const hasAutoApproveGitOp = this.AUTO_APPROVE_GIT_OPERATIONS.some(op => 
          args.includes(op) || argsString.includes(op)
        );
      ...
      if (this.isFileOperation(command, args) && !this.isDestructiveFileOperation(operation)) {
        ...
        const isSafeFileOperation = args.some(arg =>
          safeFilePatterns.some(pattern => arg.includes(pattern))
        );
      ...
      if (command === 'vercel') {
        const hasAutoApproveVercelOp = this.AUTO_APPROVE_VERCEL_OPERATIONS.some(op => 
          this.matchesPattern(argsString, op)
        );
      ...
      if (this.isScriptExecution(command, args)) {
        // .mjsファイルの実行
        const hasMjsScript = args.some(arg => 
          this.AUTO_APPROVE_SCRIPT_EXTENSIONS.some(ext => arg.endsWith(ext))
        );

        // 安全なスクリプトパスの実行
        const hasSafeScriptPath = args.some(arg =>
          this.AUTO_APPROVE_SCRIPT_PATHS.some(path => arg.startsWith(path))
        );
        ...
        if (isNpmRun) {
          ...
          const hasSafeScript = args.some(arg => 
            safeNpmScripts.includes(arg)
          );
   ...
   */
  private isReadOnlyFileOperation(command: string): boolean {
  ...
  private matchesPattern(text: string, pattern: string): boolean {
    try {
      ...
      const basicMatch = patternParts.every(patternPart => 
        textParts.some(textPart => textPart.includes(patternPart))
      );
   ...
   * 複雑な操作パターンの検出に使用
   */
  private matchesAdvancedPattern(operation: Operation, patterns: string[]): boolean {
    const command = operation.command.toLowerCase();
    const args = operation.args.map(arg => arg.toLowerCase());
    const argsString = args.join(' ');
