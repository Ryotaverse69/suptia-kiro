...
export class MetricsCollector {
    ...
    async aggregateMetrics(startDate, endDate) {
        ...
        const normalOperations = processingTimes.filter(t => t >= this.config.performanceThresholds.fast && t < this.config.performanceThresholds.normal).length;
     ...
     * 空の集計データを作成
     */
    createEmptyAggregation(startDate, endDate) {
        return {
            period: {
                start: startDate.toISOString(),
                end: endDate.toISOString()
     ...
     * トレンド分析を生成
     */
    generateTrendAnalysis(dailyMetrics) {
     ...
     * 推奨アクションを生成
     */
    generateRecommendations(weeklyMetrics, dailyMetrics) {
    ...
    async getCurrentMetrics() {
        ...
        // アラート条件のチェック
        let alertsCount = 0;
        if (todayMetrics.autoApprovalRate < 95)
            alertsCount++;
        if (recentAverageProcessingTime > 100)
            alertsCount++;
        if (todayMetrics.trustDialogDisplayCount > 20)
            alertsCount++;
        return {
