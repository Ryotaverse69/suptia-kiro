...
describe('ScoreDisplay', () => {
  ...
  describe('色分け機能', () => {
    it('スコアに応じた色分けクラスが適用される', () => {
      ...
      expect(totalScoreElement).toHaveClass('text-primary-600', 'bg-primary-100');
    });
    ...
    it('個別スコアにも適切な色分けが適用される', () => {
      ...
      expect(safetyElement).toHaveClass('text-primary-600', 'bg-primary-100');
  ...
  describe('視覚的インジケーター', () => {
    it('スコアレベルに応じたアイコンが表示される', () => {
      ...
      const goodIndicator = document.querySelector('.text-primary-600');
    ...
    it('プログレスバーの視覚的表現が正確である', () => {
      ...
      // プログレスバー自体が色分けクラスを持っている（良好レベルの場合）
      expect(totalProgressBar).toHaveClass('bg-primary-600');
