...
import { checkText, type ComplianceResult } from '../lib/compliance-client';
import {
  checkPersonaRules,
...
export function PersonaWarnings({
  ...
  const performWarningChecks = useCallback(async () => {
    ...
    try {
      ...
      const [complianceResult, personaResult] = await Promise.all([
        ...
        Promise.resolve(checkPersonaRules(product as any, userPersona)).catch(
          error => {
  ...
  const combinedWarnings = useMemo(() => {
    ...
    warningState.complianceWarnings,
    warningState.personaWarnings,
    warningState.dismissedWarnings,
  ]);
  ...
  if (warningState.error) {
    return (
      ...
      >
        <p className='text-sm text-gray-600'>{warningState.error}</p>
      </div>
    );
  }
  ...
  if (warningState.isLoading) {
    return (
      <div
        className={`p-4 bg-gray-50 border border-gray-200 rounded-md ${className}`}
      >
        <p className='text-sm text-gray-600'>警告をチェック中...</p>
      </div>
    );
  }
      ...
      {combinedWarnings
        ...
        .map(warning => {
          ...
          return (
            <div
              key={warning.id}
              className={`rounded-lg p-4 border ${style.wrap}`}
            >
  ...
  if (product.ingredients && Array.isArray(product.ingredients)) {
    product.ingredients.forEach(ingredientItem => {
      if (ingredientItem && ingredientItem.ingredient) {
        ...
        if (
          typeof ingredientItem.ingredient === 'object' &&
