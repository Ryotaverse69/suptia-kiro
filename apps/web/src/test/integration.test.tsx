...
describe('統合テスト', () => {
  ...
  describe('商品比較フロー統合テスト', () => {
    it('商品比較機能が適切に動作する', async () => {
      const testProducts: ComparisonProduct[] = [
        {
          id: 'compare-1',
          name: 'テストサプリメント',
          brand: 'Test Brand',
        ...
        },
        {
          ...
          imageUrl: '/placeholders/product-placeholder.svg',
          sellers: [
            { site: 'iHerb', price: 3480, url: '#' },
            { site: 'Amazon', price: 3580, url: '#' },
          ],
        },
      ];

      renderWithProviders(<ComparisonTable products={testProducts} />);
      ...
      await waitFor(() => {
        expect(
          screen.getAllByRole('link', { name: '最安値で購入' })
        ).toHaveLength(2);
      });
    });
  });
  ...
  describe('価格機能統合テスト', () => {
    it('価格表示と計算が正確に動作する', () => {
      renderWithProviders(
        <PriceTable products={[mockProduct]} currency='JPY' locale='ja' />
      );
  ...
  describe('国際化統合テスト', () => {
    it('言語切替が適切に動作する', () => {
      renderWithProviders(
        <PriceTable products={[mockProduct]} currency='USD' locale='en' />
      );
